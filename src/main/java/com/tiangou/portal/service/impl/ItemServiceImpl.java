...
public class ItemServiceImpl implements ItemService {
	...
	public Item getItemById(long itemId) {
		...
		//转换成tiangouresult
		TiangouResult result = TiangouResult.formatToPojo(jsonResult, Item.class);
	...
	public String getItemDesc(long itemId) {
		...
		//把json转换成java对象
		TiangouResult result = TiangouResult.formatToPojo(jsonData, TbItemDesc.class);
		if (null != result && result.getStatus() == 200) {
	...
	public String getItemParam(long itemId) {
		...
		//转换成taotaoResult对象
		TiangouResult result = TiangouResult.formatToPojo(jsonData, TbItemParamItem.class);
		//请求出错，返回空串
		if (null == result || result.getStatus() != 200) {
			return "";
		}
